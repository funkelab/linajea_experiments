...
def write_results(label_to_points, outdir):
    logger.info("Writing results to %s" % outdir)
    os.makedirs(outdir, exist_ok=True)
    for label in label_to_points.keys():
        filename = os.path.join(outdir, label + '.txt')
        with open(filename, 'w') as f:
            for point in label_to_points[label]:
                string = map(str, point)
                f.write(' '.join(string))
                f.write('\n')
...
if __name__ == '__main__':
    ...
    write_results(label_to_points, args.outdir)
