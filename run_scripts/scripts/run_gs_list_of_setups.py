...
setups = [
#    "setup211_simple_eval_side_1",
    "setup211_simple_eval_side_2"
...
def check_job_failure_loop(retries, jobid, num_configs):
    if not bmonitor.is_ended(jobid, array=True):
        logger.warn("Bjobs not yet reporting completion."
                    " Trying again in 5 seconds")
        time.sleep(5)
    assert bmonitor.is_ended(jobid, array=True)
    if bmonitor.is_done(jobid, array=True):
        return 0
...
if __name__ == '__main__':
    ...
    for setup in setups:
        ...
        # run array job on config files
        # TODO: get all configs from file
        argument = os.path.join(config_dir, "\\$LSB_JOBINDEX")
        ...
        log_file = os.path.join("logs", '.'.join(
            [job_name, "%J", "%I", "summary"]))
        error_file = os.path.join("logs", '.'.join(
            [job_name, "%J", "%I", "error"]))
        first_config = load_config(os.path.join(config_dir, '1'))
        num_cpus = 8
        array_limit = 60
        time_limit = 5  # hours
