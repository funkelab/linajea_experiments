...
def mk_net(config, output_folder):
    ...
    #     if config.train.cyclic_lr:
    #         step_size = config.train.cyclic_stepsize
    #         max_lr = config.optimizerTF1.learning_rate * 5
    #         learning_rate = config.optimizerTF1.learning_rate / 10
    #         gamma = 0.995
    #         # step = tf.subtract(step, config.train.cyclic_offset
    #         mode = config.train.cyclic_mode
    ...
    #         arg2 = step * (tf.cast(config.train.warmup_steps,tf.float32) ** -1.5)
    #         fac = tf.math.rsqrt(tf.cast(config.train.warmup_fac, tf.float32))

    #         warmup_lr = tf.multiply(fac, tf.math.minimum(arg1, arg2),
    #                                 name="warmup_lr")
    #         warmup_lr = tf.Print(warmup_lr, [warmup_lr], "warmup_lr", first_n=10)
    #         print(warmup_lr)
    #     if config.train.cyclic_lr and config.train.warmup_steps:
    #         learning_rate = tf.cond(tf.less(step, config.train.warmup_steps),
    #                                 lambda: warmup_lr,
    #                                 lambda: cyclic_lr,
    #                                 name="learning-rate")
    #         print(learning_rate)
    #     elif config.train.cyclic_lr:
    #         learning_rate = tf.identity(cyclic_lr, name="learning-rate")
    #     elif config.train.warmup_steps:
    #         learning_rate = tf.identity(warmup_lr, name="learning-rate")
    # # else:
    # #     learning_rate = tf.placeholder_with_default(config.train.learning_rate, shape=(),
    # #                                                 name="learning-rate")
